...
function include_config_and_defaults {
  ...
  CLOUD_ROBOTICS_INGRESS_IP=${CLOUD_ROBOTICS_INGRESS_IP:-}
...
function terraform_init {
  if [[ "${PROVISIONED_BY_DEPLOY_SCRIPT}" == "false" ]]; then
    die "ERROR: This project is managed by Terraform. Running terraform via deploy.sh is blocked. Please use Terraform directly."
  fi
...
terraform {
  ...
  # to consistently manage it from here now.
  import {
    to = module.cloud.google_storage_bucket.config_store
    id = "\${data.google_project.project.project_id}-cloud-robotics-config"
  }
}
EOF
  else
...
function helm_main_region {
  local INGRESS_IP
  if [[ -n "${CLOUD_ROBOTICS_INGRESS_IP}" ]]; then
    INGRESS_IP="${CLOUD_ROBOTICS_INGRESS_IP}"
  else
    if ! INGRESS_IP=$(terraform_exec output -raw ingress-ip 2>/dev/null); then
      INGRESS_IP=""
    fi
    # Validate IP format to reject terraform warnings if they were printed to stdout.
    if [[ ! "${INGRESS_IP}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      if [[ -n "${INGRESS_IP}" ]]; then
        echo "WARNING: Invalid ingress IP from terraform: '${INGRESS_IP}'. Defaulting to empty."
      fi
      INGRESS_IP=""
    fi
  fi
...
function helm_additional_region {
  ...
  local INGRESS_IP
  if [[ -n "${CLOUD_ROBOTICS_INGRESS_IP_AR[${CLUSTER_NAME}]+x}" ]]; then
    INGRESS_IP="${CLOUD_ROBOTICS_INGRESS_IP_AR[${CLUSTER_NAME}]}"
  else
    # Fallback to terraform, handle errors gracefully
    if ! INGRESS_IP=$(terraform_exec output -json ingress-ip-ar 2>/dev/null | jq -r ."\"${CLUSTER_NAME}\"" 2>/dev/null); then
      INGRESS_IP=""
    fi
    if [[ "${INGRESS_IP}" == "null" || ! "${INGRESS_IP}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      if [[ -n "${INGRESS_IP}" && "${INGRESS_IP}" != "null" ]]; then
        echo "WARNING: Invalid ingress IP for ${CLUSTER_NAME} from terraform: '${INGRESS_IP}'. Defaulting to empty."
      fi
      INGRESS_IP=""
    fi
  fi
...
function create {
  ...
  if [[ "${PROVISIONED_BY_DEPLOY_SCRIPT}" == "false" ]]; then
    echo "Skipping terraform_apply because this project is not managed by the deployment script."
  else
    terraform_apply
  fi
  helm_charts
