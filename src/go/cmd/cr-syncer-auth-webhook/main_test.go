...
func TestProxyKubernetes_Unauthorized(t *testing.T) {
...
func TestProxyKubernetes_Forbidden(t *testing.T) {
	...
	*tokenVendor = tokenVendorServer.URL
	...
	if rec.Code != http.StatusForbidden {
		t.Errorf("proxyKubernetes returned status %d, want %d", rec.Code, http.StatusForbidden)
	}
}
...
func TestProxyKubernetes_Success(t *testing.T) {
	...
	*k8sTokenPath = tokenFile
	...
	if rec.Code != http.StatusOK {
		t.Errorf("proxyKubernetes returned status %d, want %d", rec.Code, http.StatusOK)
	}
