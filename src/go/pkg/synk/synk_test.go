...
func createResourceSet(t *testing.T, rsClient crcapi.Interface, name, version string) {
	t.Helper()
	if _, err := rsClient.AppsV1alpha1().ResourceSets().Create(context.TODO(), &apps.ResourceSet{
...
func createFailedResourceSet(t *testing.T, rsClient crcapi.Interface, name, version string) {
	t.Helper()
	if _, err := rsClient.AppsV1alpha1().ResourceSets().Create(context.TODO(), &apps.ResourceSet{
		ObjectMeta: metav1.ObjectMeta{
			Name:   name + "." + version,
		...
		Status: apps.ResourceSetStatus{
			Phase: apps.ResourceSetPhaseFailed,
		},
	}, metav1.CreateOptions{}); err != nil {
		t.Error(err)
	}
...
func TestSynk_deleteResourceSets(t *testing.T) {
	...
	createResourceSet(t, synk.rsClient, "bad_name", "")
	createResourceSet(t, synk.rsClient, "other", "v3")
	createResourceSet(t, synk.rsClient, "test", "v2")
	createResourceSet(t, synk.rsClient, "test", "v4")
	createResourceSet(t, synk.rsClient, "test", "v7")
	createResourceSet(t, synk.rsClient, "test", "v8")
...
func TestSynk_deleteFailedResourceSets(t *testing.T) {
	...
	synk := f.newSynk()
	rsClient := synk.rsClient.AppsV1alpha1().ResourceSets()
	createFailedResourceSet(t, synk.rsClient, "test", "v2")
	createResourceSet(t, synk.rsClient, "test", "v4")
	createFailedResourceSet(t, synk.rsClient, "test", "v6")
	createFailedResourceSet(t, synk.rsClient, "test", "v7")
	createFailedResourceSet(t, synk.rsClient, "test", "v8")
	wantNames := []string{"test.v7", "test.v8"}
	wantDeletions := []string{"test.v2", "test.v6"}
