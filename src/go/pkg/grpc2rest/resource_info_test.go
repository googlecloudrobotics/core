...
func TestRepositoryUpdate(t *testing.T) {
	...
	hasGet := func() bool {
		m, err := r.GetMethod("/cloudrobotics.hello_world.v1alpha1.K8sHelloWorld/Get")
...
func TestRequestHasCorrectMessageTypes(t *testing.T) {
	...
	for _, tt := range messageTypeTests {
		m := methods[fmt.Sprintf("/cloudrobotics.hello_world.v1alpha1.K8sHelloWorld/%s", tt.method)]
...
func TestKubernetesRequestHandlesHttpError(t *testing.T) {
	...
	m := methods["/cloudrobotics.hello_world.v1alpha1.K8sHelloWorld/List"]
...
func TestStreamWorks(t *testing.T) {
	g := NewGomegaWithT(t)
	methods, transport := createMethodsOrDie(crdtypes.NamespaceScoped)
	m := methods["/cloudrobotics.hello_world.v1alpha1.K8sHelloWorld/Watch"]
	transport.responseCode = http.StatusOK
